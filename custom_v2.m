...
Rx_xyz=[26,1,1];                % Rx coordinates (in meters)
%RIS_xyz=[5,2,2];       % RIS coordinates (in meters)
Nsym = 100;
shifts = 48;
hei = 6;         %keep hei=2 for finding spread
I = eye(Nr);
H = zeros(N,Nt,Nsym);
G = zeros(Nr,N,Nsym);
D = zeros(Nr,Nt,Nsym);
...
hold off
xlim([1 26])
legend({'RIS, z=1.33 m','RIS, z=1.66 m','RIS, z=2 m','RIS, z=2.33 m','RIS, z=2.66 m','RIS, z=3 m','No RIS'},'Location','north')
